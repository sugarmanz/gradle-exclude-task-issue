...
class AggregateGroovyPlugin extends PluginHelper implements Plugin<Project> {

    static final String RELEASE_GROUP = 'Aggregate'

    void apply(Project project) {
        this.project = project
        extension = project.extensions.create('release', AggregateExtension, project, attributes)
        ...
        project.task('first', group: RELEASE_GROUP,
                description: 'Finds the correct SCM plugin') doLast this.&first
        project.task('second', group: RELEASE_GROUP,
                description: 'Finds the correct SCM plugin') doLast this.&second
        ...
        if (supportsMustRunAfter) {
            project.tasks.second.mustRunAfter(project.tasks.first)
            project.tasks.createScmAdapter.mustRunAfter(project.tasks.second)
    ...
    void createScmAdapter() {
        println("createScmAdapter")
    }

    void initScmAdapter() {
        println("initScmAdapter")
    }
